<template>
    <div class="content component photo-gallery" v-if="gallery && Object.keys(gallery).length">
        ...
        <div class="photo-gallery__date_container" v-show="show">
            <div v-for="(PhotoArray, date) in gallery" v-bind:key="date" v-if="PhotoArray && PhotoArray.length">
                <h4 class="photo-gallery__date">{{date}}</h4>
                <div class="photo-gallery__container">
                    <a v-for="(item, i) in PhotoArray" v-bind:href="item.url" v-bind:key="i" data-fancybox="gallery" v-bind:data-caption="'Загружено ' + date" class="photo-gallery__item">
                        <img v-bind:src="(item.thumbnail) ? item.thumbnail : item.url" />
                    </a>
                </div>
            </div>
        </div>
